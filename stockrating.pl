...
print_to($file, 'Ticker', 'Sector', 'Latest report date', 'Last report date',  'Market beat value', 'Market reaction',  'Earning surprise', 'Gross margin change', 'Margin trend', 'Revenue growth trend', 'Peg', 'Peg/Peg of peers', 'Peg/Peg Sector', 'CAPE', 'Current market performance', 'Enterprise/Cash compareed with peers');	
...
for my $i (0 .. $#ticker_list) {
    ...
    print_to($file, $ticker_array[3*$i], $sector, $next_report_date, $last_report_date,  $beat_value, $market_reaction,  $earning_surprise, $gross_margin_ratio[4], $margin_trend, $revenue_growth_trend, $peg, $peg_avg_competitors, 'N/A', 'N/A', $cur_market_performance, $stock_competitors_enterprise_cash_ratio);
...
sub print_to{
	...
	my @row = ($ticker, $sector, $next_report_date, $last_report_date, $beat_value, $market_reaction, $earning_surprise, $gross_margin_ratio, $margin_trend, $revenue_growth_trend, $peg, $peg_avg_competitors, $peg_to_sector_ratio, $cape_performance, $cur_market_performance, $stock_competitors_enterprise_cash_ratio);
